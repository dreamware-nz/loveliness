...
COPY --from=builder /app/deploy/fly/start.sh /usr/local/bin/start.sh

RUN useradd -r -s /bin/false loveliness && mkdir -p /data && chown loveliness:loveliness /data && chmod +x /usr/local/bin/start.sh
USER loveliness
